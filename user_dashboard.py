...
# Step 3: Ranking users based on session duration and clusters
df['RANK'] = df.groupby('CLUSTER')['SESSION_DURATION'].rank(ascending=False)

# Step 4: Build Dashboard with Dash
app = dash.Dash(__name__)

app.layout = html.Div([
    html.H1("User Clustering Dashboard", style={'textAlign': 'center'}),
...
def update_cluster_graph(selected_orgs):
    ...
    fig = px.scatter(filtered_df, x='USR_ID', y='SESSION_DURATION', color='CLUSTER',
                     ...
                     title="User Clustering based on Session Duration and Organization")
    return fig
